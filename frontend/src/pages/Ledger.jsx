...
// Always display zero values instead of leaving Debit/Credit cells blank.
...
function Ledger() {
  ...
  function handleViewStatement(event) {
    ...
    const match = ledgers.find(
      (ledger) => ledger.name.toLowerCase() === trimmed.toLowerCase()
    )

    if (!match) {
      setSelectionError(
        'Pick a ledger from the suggestions list before viewing its statement.'
      )
      return
    }
  ...
  const report = reportResponse?.success
    ...
    ? [
        {
          ...
          credit:
            report.opening_balance < 0
              ? Math.abs(report.opening_balance)
              : 0,
  ...
  return (
    <>
      <PageHeader
        title="Ledger"
        subtitle="View any ledger's statement in Tally's format and export it"
      />
      ...
      {!activeLedger && (
        <Card>
          <p className="empty-state">
            Select a ledger above and click "View Statement" to
            see its statement.
          </p>
        </Card>
      )}
      ...
      {report && (
        <Card title={`Ledger: ${report.ledger_name}`}>
          <p className="card-note ledger-summary-note">
            Period:{' '}
            {activeLedger.from || 'books beginning'} to{' '}
            {activeLedger.to || 'today'}
            {' · '}
            Opening Balance:{' '}
